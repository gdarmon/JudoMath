...
describe('MainMenu', () => {
  it('renders the title', () => {
    ...
    expect(screen.getByText('Judo Math')).toBeInTheDocument();
  });
  ...
  it('displays stripes when currentStripes > 0', () => {
    render(<MainMenu onNavigate={vi.fn()} currentBelt={Belt.Yellow} currentStripes={2} />);
    const stripesEl = screen.getByLabelText('2 stripes');
    expect(stripesEl).toBeInTheDocument();
  });

  it('does not display stripes when currentStripes is 0', () => {
    render(<MainMenu onNavigate={vi.fn()} currentBelt={Belt.Blue} currentStripes={0} />);
    expect(screen.queryByLabelText(/stripe/)).not.toBeInTheDocument();
  });
