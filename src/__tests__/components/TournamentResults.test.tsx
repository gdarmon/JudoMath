...
describe('TournamentResults', () => {
  ...
  it('renders the tournament results region', () => {
    ...
    expect(
      screen.getByRole('region', { name: TOURNAMENT_RESULTS.regionLabel }),
    ).toBeInTheDocument()
  })
  ...
  it('shows celebration header for scores >= 75%', () => {
    render(<TournamentResults {...defaultProps} />)
    const header = screen.getByText(TOURNAMENT_RESULTS.amazing).closest('.tournament-results__header')
  ...
  it('calls onMainMenu when Main Menu button is clicked', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: TOURNAMENT_RESULTS.mainMenu }))
    expect(onMainMenu).toHaveBeenCalledTimes(1)
  })

  it('formats time correctly for values under a minute', () => {
    const quickResult: TournamentResult = {
      totalScore: 90,
      totalTime: 45000,
      sessionsCompleted: 3,
    }
  ...
  it('formats time correctly for values over 10 minutes', () => {
    const slowResult: TournamentResult = {
      totalScore: 60,
      totalTime: 720000,
      sessionsCompleted: 3,
    }
